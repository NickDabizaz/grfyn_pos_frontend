...
export default function ProductCatalog({ allProducts, getStock, loadProducts }) {
  ...
  const handleAddItem = (product) => {
    ...
    // Validasi stok <= 0 (atau tidak cukup)
    if (stok - currentCartQty <= 0) {
      toast.error(`Stok barang ${product.namabarang} tidak mencukupi!`);
      return;
    }
  ...
  const handleKeyDown = async (e) => {
    if (e.key === 'Enter') {
      // Barcode Scanner Support: Jika hasil fetch tepat 1, otomatis masukkan keranjang
