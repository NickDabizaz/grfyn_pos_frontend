...
export default function PenyesuaianStokForm({ onSuccess, tabId }) {
  const [barangList, setBarangList] = useState([]);
  const [items, setItems] = useState([{ idbarang: '', jml: '', keterangan: '' }]);
  ...
  const addRow = () => setItems([...items, { idbarang: '', jml: '', keterangan: '' }]);
  ...
  const handleSave = async (e) => {
    ...
    setLoading(true);
    try {
      await api.post('/stok/penyesuaian', {
        keterangan,
        items: items.map(d => ({ idbarang: parseInt(d.idbarang), jml: parseInt(d.jml), keterangan: d.keterangan }))
      });
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-3 px-6 pt-4 pb-2 border-b border-primary-50 shrink-0">
        ...
        <div><h2 className="text-lg font-bold text-dark-500">Penyesuaian Stok</h2><p className="text-xs text-dark-300">Opname stok fisik vs sistem</p></div>
      </div>
      <div className="flex-1 overflow-auto p-6">
        <form onSubmit={handleSave} className="max-w-3xl space-y-4">
          <div>
            ...
            <input value={keterangan} onChange={e => setKeterangan(e.target.value)} className="w-full px-3 py-2.5 rounded-xl border border-primary-100 text-sm" placeholder="Opname stok..." />
          </div>
          <div className="space-y-2">
            {items.map((d, i) => (
              <div key={i} className="flex items-end gap-2 p-3 rounded-xl bg-warm-50/50">
                ...
                <div className="w-32">
                  <label className="block text-[10px] font-semibold text-dark-300 mb-1">Fisik</label>
                  <input type="number" value={d.jml} onChange={e => updateRow(i, 'jml', e.target.value)} className="w-full px-3 py-2 rounded-lg border border-primary-100 text-sm text-center" min="0" placeholder="Qty fisik" />
                </div>
                <div className="flex-1">
                  <label className="block text-[10px] font-semibold text-dark-300 mb-1">Catatan</label>
                  <input value={d.keterangan} onChange={e => updateRow(i, 'keterangan', e.target.value)} className="w-full px-3 py-2 rounded-lg border border-primary-100 text-sm" placeholder="Catatan" />
                </div>
